    ...
    <li className="flex flex-col items-center gap-2">
      ...
      >
        <div className="rounded-xl w-14 h-14 flex items-center justify-center bg-black bg-opacity-30">
          {text ? (
            <p style={poppins.style}  className="font-bold text-2xl text-white">{text}</p>
          ) : img ? (
      ...
      {label && <p style={poppins.style} className="text-sm">{label}</p>}
    </li>
  )
}
...
const BadgeGrouping = ({
  ...
  return (
    <div className="bg-black py-6 px-4 rounded-2xl mt-8">
      <p style={poppins.style} className="text-xl font-bold pb-4 text-center">{label}</p>
...
function TechnoBadges() {
  return (
    <section className="my-8 flex flex-col">
      ...
      >
        <p style={poppins.style} className="border-b border-[#646cff] arrow-animation">
          &nbsp;&nbsp;Download CV
        </p>
      </a>
    </section>
